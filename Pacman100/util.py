...
def getStateRepresentation(state):
    ...
    allSides2 = [[0, 1], [0, 2], [0, -1], [0, -2], [1, 0], [2, 0], [-1, 0], [-2, 0]]
    allSides3 = [[-1, 1], [1, 1], [1, -1], [-1, -1]]
    capsules = state.getCapsules()
    for side in allSides2:
        
        currentX = pacmanX + side[0]
        currentY = pacmanY + side[1]

        if state.hasWall(pacmanX+int(side[0]/2), pacmanY+int(side[1]/2)) or state.hasWall(currentX, currentY):
            result.append(0)
            result.append(0)
        # no wall
        else:
    ...
    direction = Actions._directions[state.getPacmanState().getDirection()]
    ...
    if state.getPacmanState().getDirection() == Directions.STOP:
      ...
      for i in range(10):
        ...
        x = pacmanX + direction[0]*(i+1)
        y = pacmanY + direction[1]*(i+1)

        if state.hasWall(x, y):
          break

        for j, ghostPosition in enumerate(ghostsPositions):
          if x == ghostPosition[0] and y == ghostPosition[1]:
            if not ghostsScaredTimers[j] > 0:
              if Directions.REVERSE[state.getGhostState(j+1).getDirection()] == state.getPacmanState().getDirection():
                toAdd = 1
                break
